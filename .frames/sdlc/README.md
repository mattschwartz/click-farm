...
bash tools/board.sh --role <role>
```

At the beginning of a session, you MUST run `tools/board.sh` to understand the current state of the project. To see your specific work, run `tools/board.sh --role <your-role>`.
...
```bash
# List all open tasks
bash tools/task.sh list

# List open tasks for a role
bash tools/task.sh list --role <role>

# List all tasks including completed
bash tools/task.sh list --all

# Get a single task by ID
bash tools/task.sh get <id>
```

### Creating a task

```bash
bash tools/task.sh add --role <target-role> <<'EOF'
{
...
```bash
bash tools/task.sh complete <id>
```
...
You will occasionally need other agents to help complete your task. You SHOULD rely on your team when necessary to deliver the best product for customers. To create work for another agent, use `tools/task.sh add --role <target-role>` with a JSON body on stdin.
