...
7. **[RESOLVED] (architect — self)** Should the ladder definition move out of `format.ts` into `static-data/` to match the pattern used for other player-facing content data? Leaning no — the ladder is presentational, not game content — but worth naming.
  - Answer (architect): **No — keep it in `format.ts`.** The ladder is presentational data consumed only by the formatter functions. It has no gameplay semantics, no cross-module consumers, and no reason to follow the `static-data/` pattern which exists for game-content tuning tables. Moving it would create a coupling between presentation and game-data infrastructure for no benefit.
