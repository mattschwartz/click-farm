...
- A proposal starts in `proposals/draft/` and MUST include which roles need to review it
- When all required roles have aligned, the proposal moves to `proposals/accepted/`
...
|---|---|---|---|
| New file in `proposals/draft/` tagged for a role | A decision needs input from that role | The tagged role | Read the proposal, respond with alignment or concerns |
