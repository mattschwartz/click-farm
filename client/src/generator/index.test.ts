...
function stateWithOwnedGenerator(
  ...
  count: number,
  level: number = 1,
  engagement: Decimal | number = 0,
  autoclicker_count?: number,
): GameState {
  const base = createInitialGameState(STATIC_DATA, T0);
  return {
    ...base,
    player: earnEngagement(base.player, new Decimal(engagement)),
    generators: {
      ...base.generators,
      [id]: {
...
describe('generatorBuyCost', () => {
  it('returns the base cost when count is 0', () => {
    ...
    expect(cost).toEqualDecimal(STATIC_DATA.generators.selfies.base_buy_cost);
  });

  it('increases monotonically with each unit owned', () => {
    for (let n = 0; n < 10; n++) {
      expect(generatorBuyCost('selfies', n + 1, STATIC_DATA).gt(
  ...
  it('applies buy_cost_multiplier per unit owned', () => {
    ...
    expect(generatorBuyCost('selfies', 5, STATIC_DATA).toNumber()).toBeCloseTo(expected);
  });
...
describe('generatorUpgradeCost', () => {
  it('returns the first entry in upgrade_costs at level 1', () => {
    ...
    expect(generatorUpgradeCost('selfies', 1, STATIC_DATA)).toEqualDecimal(costs[0]);
  });

  it('returns correct entry for each level', () => {
    const costs = STATIC_DATA.generators.selfies.upgrade_costs;
    for (let l = 1; l <= costs.length; l++) {
      expect(generatorUpgradeCost('selfies', l, STATIC_DATA)).toEqualDecimal(costs[l - 1]);
    }
  });
  ...
  it('increases monotonically with level', () => {
    for (let l = 1; l < 8; l++) {
      expect(
        generatorUpgradeCost('selfies', l + 1, STATIC_DATA).gt(
          generatorUpgradeCost('selfies', l, STATIC_DATA),
        ),
      ).toBe(true);
    }
  });
...
describe('checkGeneratorUnlocks', () => {
  ...
  it('auto-unlocks passive-only generators in the correct progressive order', () => {
    ...
    // Manual-clickable ladder verbs auto-unlock via checkGeneratorUnlocks.
...
describe('buyGenerator', () => {
  ...
  it('deducts the correct cost from engagement', () => {
    const cost = generatorBuyCost('selfies', 0, STATIC_DATA);
    const state = stateWithOwnedGenerator('selfies', 0, 1, cost.plus(500));
    const next = buyGenerator(state, 'selfies', STATIC_DATA);
    expect(next.player.engagement).toEqualDecimal(500);
  });
  ...
  it('successive purchases cost progressively more', () => {
    ...
    state = buyGenerator(state, 'selfies', STATIC_DATA);
    expect(engagementBefore.minus(state.player.engagement).toNumber()).toBeCloseTo(cost1.toNumber());
  });
  ...
  it('newly-bought generator increases yield multiplier (1+count)', () => {
    ...
    expect(rateAfter.toNumber()).toBeCloseTo(rateBefore.times(2).toNumber(), 6);
  });
  ...
  it('does not mutate the input state', () => {
    ...
    const countBefore = state.generators.selfies.count;
    buyGenerator(state, 'selfies', STATIC_DATA);
    expect(state.player.engagement.toString()).toBe(engBefore);
    expect(state.generators.selfies.count).toBe(countBefore);
  });
...
describe('upgradeGenerator', () => {
  ...
  it('deducts the correct cost from engagement', () => {
    const cost = generatorUpgradeCost('selfies', 1, STATIC_DATA);
    const state = stateWithOwnedGenerator('selfies', 1, 1, cost.plus(999));
    const next = upgradeGenerator(state, 'selfies', STATIC_DATA);
    expect(next.player.engagement).toEqualDecimal(999);
  });
  ...
  it('cost increases between successive levels (~3× per level)', () => {
    ...
    expect(level2Cost.gt(level1Cost)).toBe(true);
    expect(level2Cost.div(level1Cost).toNumber()).toBeCloseTo(3, 0);
  });
  ...
  it('higher count increases effective engagement rate via (1+count) multiplier', () => {
    ...
    expect(rate2.gt(rate1)).toBe(true);
  });
  ...
  it('rate gain matches (1+count) ratio', () => {
    ...
    // (1+3)/(1+1) = 2.0
    expect(rate2.div(rate1).toNumber()).toBeCloseTo((1 + 3) / (1 + 1), 6);
  });
  ...
  it('does not mutate the input state', () => {
    ...
    const state = stateWithOwnedGenerator('selfies', 1, 1, cost);
    const engBefore = state.player.engagement.toString();
    const levelBefore = state.generators.selfies.level;
    upgradeGenerator(state, 'selfies', STATIC_DATA);
    expect(state.player.engagement.toString()).toBe(engBefore);
    expect(state.generators.selfies.level).toBe(levelBefore);
  });
  ...
  it('chaining multiple upgrades advances level correctly', () => {
    ...
    const totalCost = cost12.plus(cost23).plus(cost34).plus(1);
...
describe('tick — generator unlocks', () => {
  ...
  it('unlocks memes once follower threshold is crossed', () => {
    ...
    expect(next.player.total_followers.toNumber()).toBeGreaterThan(
...
// ---------------------------------------------------------------------------
// buyGenerator — unowned generator error (task #120 / task #164 cleanup)
// ---------------------------------------------------------------------------

describe('buyGenerator — unowned generator error', () => {
  it('throws "check follower thresholds" for an unowned generator', () => {
    const base = createInitialGameState(STATIC_DATA, T0);
    // selfies starts unowned (threshold=100)
...
describe('autoclickerBuyCost', () => {
  it('returns base_autoclicker_cost at count=0', () => {
    ...
    expect(cost).toEqualDecimal(STATIC_DATA.generators.chirps.base_autoclicker_cost);
  });
  ...
  it('scales exponentially: base × autoclicker_cost_multiplier^count', () => {
    ...
    expect(cost3.toNumber()).toBeCloseTo(expected);
  });

  it('increases monotonically', () => {
    for (let i = 0; i < 5; i++) {
      expect(autoclickerBuyCost('chirps', i + 1, STATIC_DATA).gt(
...
describe('buyAutoclicker', () => {
  ...
  it('deducts the correct escalated cost at higher counts', () => {
    ...
    const totalNeeded = cost0.plus(cost1).plus(1);
    ...
    expect(state.player.engagement).toEqualDecimal(1);
  });
