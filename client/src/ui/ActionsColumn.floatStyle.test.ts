...
describe('floatStyle', () => {
  it('returns t=1.0 (max size, brightest gold) when engagement <= 1 (first tap)', () => {
    const result = floatStyle(10, 0);
    expect(result.fontSize).toBe(32);
    expect(result.color).toBe('hsl(43, 100%, 71%)');
  });
  ...
  it('returns t=0.0 (min size, dimmest gold) at ratio 1e-6', () => {
    // ratio = 1 / 1_000_000 = 1e-6 → log10 = -6 → t = (-6+6)/6 = 0
    const result = floatStyle(1, 1_000_000);
    expect(result.fontSize).toBe(16);
    expect(result.color).toBe('hsl(43, 40%, 52%)');
  });
  ...
  it('returns midpoint values at ratio 0.001 (t ≈ 0.5)', () => {
    ...
    expect(result.color).toBe('hsl(43, 70%, 61.5%)');
  });
  ...
  it('scales continuously between floor and ceiling', () => {
    ...
    const expectedSat = 40 + (100 - 40) * expectedT;
    const expectedLight = 52 + (71 - 52) * expectedT;
    expect(result.fontSize).toBeCloseTo(expectedSize, 5);
    expect(result.color).toBe(`hsl(43, ${expectedSat}%, ${expectedLight}%)`);
  });
});
